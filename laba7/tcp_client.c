...
int reliable_send(MessageEx *m)
{
    ...
    {
        if (send_all_ssl(ssl, m, sizeof(*m)) < 0)
            return 0;
        
        printf("[Security][ENC] SSL_write %s (id=%u)\n", 
               (m->type == MSG_PING) ? "MSG_PING" : "message", m->msg_id);
...
int connect_server_ssl()
{
    ...
    printf("[Transport] connected to server\n");
...
int auth_user()
{
    ...
    send_all_ssl(ssl, &m, sizeof(m));
    if (recv_all_ssl(ssl, &m, sizeof(m)) <= 0) return -1;
...
void* receiver(void *arg)
{
    ...
    while (1)
    {
        int r = recv_all_ssl(ssl, &m, sizeof(m));
        if (r <= 0)
        {
            connected = 0;
        ...
        else if (m.type == MSG_ERROR || m.type == MSG_SECURE_ERROR)
        {
            printf("\n[ERROR]: %s\n> ", m.payload);
        }
...
void ping_once(int seq)
{
    ...
    send_all_ssl(ssl, &m, sizeof(m));
    printf("[Security][ENC] SSL_write MSG_PING (id=%u)\n", m.msg_id);
}
